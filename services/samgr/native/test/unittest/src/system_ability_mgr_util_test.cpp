 ...
 * @tc.desc: test GetBoolParameter returns false when param is false.
 * @tc.type: FUNC
 */
HWTEST_F(SamgrUtilTest, CheckSupportSetDeathPrior001, TestSize.Level3)
{
    system::g_mockBoolValue = false;
    bool ret = system::GetBoolParameter("const.samgr.setdeathprior.support", false);
    EXPECT_FALSE(ret);
}

/**
 * @tc.name: CheckSupportSetDeathPrior002
 * @tc.desc: test GetBoolParameter returns true when param is true.
 * @tc.type: FUNC
 */
...
HWTEST_F(SamgrUtilTest, DeathHandlerInit002, TestSize.Level3)
{
    ...
    saMgr->Init();
    if (saMgr->deathHandler_ == nullptr) {
        saMgr->deathHandler_ = make_shared<FFRTHandler>("deathHandler", ffrt_qos_user_interactive);
    }
 ...
 * @tc.desc: test AbilityStatusDeathRecipient OnRemoteDied, sync UnSubscribeSystemAbility path
 * @tc.type: FUNC
 */
 ...
 * @tc.desc: test AbilityStatusDeathRecipient OnRemoteDied, async AsyncUnSubscribeSystemAbility path
 * @tc.type: FUNC
 */
