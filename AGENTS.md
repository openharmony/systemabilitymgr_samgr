...
| `interfaces/innerkits/samgr_proxy:samgr_proxy`　　 | 共享库　　 | 其余子系统依赖的SDK |
...
| 新增SA ID | `system_ability_definition.h`（枚举）。**必须三处同步**: ① 本文件(samgr仓) + ② `base/hiviewdfx/hidumper/frameworks/native/dump_utils.cpp`的`saNameMap_`(hidumper仓) + ③ `interfaces/innerkits/rust/src/definition.rs`(samgr仓Rust绑定) |
...
1. **SA ID三处同步**: 新增SA ID必须同时更新 ① `interfaces/innerkits/samgr_proxy/include/system_ability_definition.h`(samgr仓) ② `base/hiviewdfx/hidumper/frameworks/native/dump_utils.cpp`的`saNameMap_`(hidumper仓) ③ `interfaces/innerkits/rust/src/definition.rs`(samgr仓Rust绑定)。
...
12. **生成文件**: Rust绑定C++封装(`interfaces/innerkits/rust/src/cxx/*.cpp`,由cxx从`src/*.rs`生成)、hidumper saNameMap_(`base/hiviewdfx/hidumper/frameworks/native/dump_utils.cpp`)、`hisysevent.yaml`生成配置均为派生产物 — 编辑源文件，禁止直接修改生成输出。
...
# 主要测试目标: SystemAbilityMgrTest, BaseSystemAbilityMgrTest,
#   SystemAbilityStateSchedulerTest, SystemAbilityMgrCollectTest,
#   SystemAbilityMgrStubTest, SystemAbilityMgrDumperTest
