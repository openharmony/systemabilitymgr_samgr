...
int32_t SystemAbilityManagerProxy::SubscribeSystemProcessList(const std::list<std::u16string>& procNames,
    const sptr<ISystemProcessStatusChange>& listener)
{
    HILOGD("SubscribeSystemProcessList called");
    if (listener == nullptr) {
        HILOGE("SubscribeSystemProcessList listener is nullptr");
        return ERR_INVALID_VALUE;
    }

    sptr<IRemoteObject> remote = Remote();
    if (remote == nullptr) {
        HILOGI("SubscribeSystemProcessList remote is nullptr");
        return ERR_INVALID_OPERATION;
    }
    ...
    bool ret = data.WriteRemoteObject(listener->AsObject());
    if (!ret) {
        HILOGW("SubscribeSystemProcessList Write listenerName failed");
        return ERR_FLATTEN_OBJECT;
    }
    size_t size = procNames.size();
    ret = data.WriteInt32(size);
    if (!ret) {
        HILOGW("SubscribeSystemProcessList Write procNames size failed");
        return ERR_FLATTEN_OBJECT;
    }
    for (const auto& procName : procNames) {
        ret = data.WriteString16(procName);
        if (!ret) {
            HILOGW("SubscribeSystemProcessList Write procName failed");
            return ERR_FLATTEN_OBJECT;
        }
    }
    ...
    MessageOption option;
    int32_t err = remote->SendRequest(
        static_cast<uint32_t>(SamgrInterfaceCode::SUBSCRIBE_SYSTEM_PROCESS_LIST_TRANSACTION), data, reply, option);
    if (err != ERR_NONE) {
        HILOGE("SubscribeSystemProcessList SendRequest error:%{public}d!", err);
        return err;
    }
    HILOGD("SubscribeSystemProcessList SendRequest succeed!");
    int32_t result = 0;
    ret = reply.ReadInt32(result);
    if (!ret) {
        HILOGW("SubscribeSystemProcessList Read result failed!");
        return ERR_FLATTEN_OBJECT;
    }
    return result;
}

int32_t SystemAbilityManagerProxy::UnSubscribeSystemProcessList(const std::list<std::u16string>& procNames,
    const sptr<ISystemProcessStatusChange>& listener)
{
    HILOGD("UnSubscribeSystemProcessList called");
    if (listener == nullptr) {
        HILOGE("UnSubscribeSystemProcessList listener is nullptr");
        return ERR_INVALID_VALUE;
    }

    sptr<IRemoteObject> remote = Remote();
    if (remote == nullptr) {
        HILOGI("UnSubscribeSystemProcessList remote is nullptr");
        return ERR_INVALID_OPERATION;
    }
    ...
    bool ret = data.WriteRemoteObject(listener->AsObject());
    if (!ret) {
        HILOGW("UnSubscribeSystemProcessList Write listenerName failed");
        return ERR_FLATTEN_OBJECT;
    }
    size_t size = procNames.size();
    ret = data.WriteInt32(size);
    if (!ret) {
        HILOGW("UnSubscribeSystemProcessList Write procNames size failed");
        return ERR_FLATTEN_OBJECT;
    }
    for (const auto& procName : procNames) {
        ret = data.WriteString16(procName);
        if (!ret) {
            HILOGW("UnSubscribeSystemProcessList Write procName failed");
            return ERR_FLATTEN_OBJECT;
        }
    }
    ...
    MessageOption option;
    int32_t err = remote->SendRequest(
        static_cast<uint32_t>(SamgrInterfaceCode::UNSUBSCRIBE_SYSTEM_PROCESS_LIST_TRANSACTION), data, reply, option);
    if (err != ERR_NONE) {
        HILOGE("UnSubscribeSystemProcessList SendRequest error:%{public}d!", err);
        return err;
    }
    HILOGD("UnSubscribeSystemProcessList SendRequest succeed!");
    int32_t result = 0;
    ret = reply.ReadInt32(result);
    if (!ret) {
        HILOGW("UnSubscribeSystemProcessList Read result failed!");
        return ERR_FLATTEN_OBJECT;
    }
    return result;
}
