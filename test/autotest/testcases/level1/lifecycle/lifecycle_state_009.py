...
from hypium.action.os_hypium.device_logger import DeviceLogger, AsyncCommand
...
sa_listen_cfg_path = get_resource_path(
    "resource/level/lifecycle_state_009/listen_test.cfg",
    isdir=None)
sa_listen_json_path = get_resource_path(
    "resource/level/lifecycle_state_009/listen_test.json",
    isdir=None)
...
class LifeCycle_State_009(TestCase):
    ...
    def test_step(self):
        ...
        # 预期结果
        # 控制台打印: OnLoadSystemAbilitySuccesss systemAbilityId:1494 打印五次
        # Hilog打印: 
        #   [SA Scheduler][SA: 1494] save load event
        #   [SA Scheduler][SA: 1494] loaded
        #清理日志
        driver.shell('hilog -r')
        # 开始保存日志
        device_logger = DeviceLogger(driver).set_filter_string("Scheduler SA:1494")
        device_logger.start_log(get_report_dir() + '//lifecycle_state_009_txt')
        result = driver.System.execute_command("ondemand test 9")
        # 统计控制台字符串出现次数
        countWindowResult = result.count("OnLoadSystemAbilitySuccesss systemAbilityId:1494")
        assert countWindowResult == 5
        # 停止日志
        ...
        log_check_result1 = device_logger.check_log("Scheduler SA:1494 loaded", EXCEPTION=True)
        Assert.equal(self, log_check_result1, True)
        log_check_result2 = device_logger.check_log("Scheduler SA:1494 save load event", EXCEPTION=True)
        Assert.equal(self, log_check_result2, True)
