...
sa_listen_cfg_path = get_resource_path(
    "resource/level/lifecycle_state_008/listen_test.cfg",
    isdir=None)
sa_listen_json_path = get_resource_path(
    "resource/level/lifecycle_state_008/listen_test.json",
    isdir=None)
...
class LifeCycle_State_008(TestCase):
    ...
    def test_step(self):
        ...
        device_logger.start_log(get_report_dir() + '//lifecycle_state_008_txt')
        result = driver.System.execute_command("ondemand test 8")
        assert "OnLoadSystemAbilitySuccesss systemAbilityId:1494" in result
        # 停止日志
        device_logger.stop_log()
        # 检查日志输出
        device_logger.check_log("Scheduler SA:1499 handle load failed event", EXCEPTION=True)
