...
from devicetest.core.test_case import TestCase, get_report_dir, ACTUAL
...
sa_listen_cfg_path = get_resource_path(
    "resource/level/lifecycle_state_001/listen_test.cfg",
    isdir=None)
sa_listen_json_path = get_resource_path(
    "resource/level/lifecycle_state_001/listen_test.json",
    isdir=None)
...
class LifeCycle_State_001(TestCase):
    ...
    def test_step(self):
        ...
        # 6、再打开一个cmd用来查看hilog, 执行hilog | grep "Scheduler SA"
        # 预期结果
        # 控制台打印: OnLoadSystemAbilitySuccesss systemAbilityId:1494
        # Hilog打印: [SA Scheduler][SA: 1494] loaded
        #清理日志
        driver.shell('hilog -r')
        # 开始保存日志
        device_logger = DeviceLogger(driver).set_filter_string("Scheduler SA:1494")
        device_logger.start_log(get_report_dir() + 'lifecycle_state_001_txt')
        result = driver.System.execute_command("ondemand test 1")
        assert "OnLoadSystemAbilitySuccesss systemAbilityId:1494" in result
        # 停止日志
