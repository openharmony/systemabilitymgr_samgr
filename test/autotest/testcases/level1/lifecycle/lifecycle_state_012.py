...
class lifecycle_state_012(TestCase):
    ...
    def setup(self):
        self.log.info("lifecycle_state_012 start")
        need_source = {"cfg": True, "fwk": False, "listen_test": True, "audio_ability": False, "ondemand": True,
                       "proxy": True, "para": True}
        self.source_path = get_source_path(need_source=need_source, casename="level1/lifecycle/lifecycle_state_012")
    ...
    def test_step(self):
        ...
        device_logger.start_log(get_report_dir() + "//lifecycle_state_012.txt")
        driver.System.execute_command("ondemand test 12")
        device_logger.stop_log()
        device_logger.check_log("Scheduler SA:1494 in state 0,not need unload SA", EXCEPTION=True)

    def teardown(self):
        remove_source(source_path=self.source_path, driver=self.driver, sn=self.sn)
        self.log.info("lifecycle_state_012 down")
