...
sa_listen_cfg_path = get_resource_path(
    "resource/level/lifecycle_state_003/listen_test.cfg",
    isdir=None)
sa_listen_json_path = get_resource_path(
    "resource/level/lifecycle_state_003/listen_test.json",
    isdir=None)
...
class LifeCycle_State_003(TestCase):
    ...
    def test_step(self):
        ...
        # Hilog打印: 
        #   [SA Scheduler][SA: 1494] remove pending unload event
        #   [SA Scheduler][SA: 1494] loaded
        #清理日志
        driver.shell('hilog -r')
        # 开始保存日志
        device_logger = DeviceLogger(driver).set_filter_string("Scheduler SA:1494")
        device_logger.start_log(get_report_dir() + 'lifecycle_state_003_txt')
        result = driver.System.execute_command("ondemand test 3")
