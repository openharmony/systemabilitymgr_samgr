...
class lifecycle_state_017(TestCase):
    ...
    def setup(self):
        self.log.info("lifecycle_state_017 start")
        need_source = {"cfg": True, "fwk": False, "listen_test": True, "audio_ability": False, "ondemand": True,
                       "proxy": True, "para": True}
        self.source_path = get_source_path(need_source=need_source, casename="level1/lifecycle/lifecycle_state_017")
    ...
    def test_step(self):
        ...
        device_logger.start_log(get_report_dir() + "//lifecycle_state_017.txt")
        result = driver.System.execute_command("ondemand test")
        device_logger.stop_log()
        assert "OnLoadSystemAbilityFail systemAbilityId:1499" in result.split("input")[18]
    ...
    def teardown(self):
        ...
        self.log.info("lifecycle_state_017 down")
