...
sa_listen_cfg_path = get_resource_path(
    "resource/level/lifecycle_state_010/listen_test.cfg",
    isdir=None)
sa_listen_json_path = get_resource_path(
    "resource/level/lifecycle_state_010/listen_test.json",
    isdir=None)
...
class LifeCycle_State_010(TestCase):
    ...
    def test_step(self):
        ...
        device_logger.start_log(get_report_dir() + '//lifecycle_state_010_txt')
        log_path = os.path.join(self.get_case_report_path(), "lifecycle_state_010_txt")
        command = AsyncCommand(driver).set_output_file(log_path)
        command.start_device_command("ondemand test")
        time.sleep(130)
        # 停止后台命令
        command.stop()
        # 停止日志
        ...
        # 断言
        # 检查日志输出
        countResult = Disk_drop_log.count_keys_disk_dropping_logs(self.get_case_report_path(),
                                                                    "OnLoadSystemAbilitySuccesss systemAbilityId:1494")
        assert countResult >= 23
