...
class case12_delay_time001(TestCase):
    ...
    def setup(self):
        self.log.info("case12_delay_time001 start")
        need_source = {"cfg": True, "fwk": False, "listen_test": True, "audio_ability": True, "ondemand": True,
                       "proxy": True, "para": False}
        self.source_path = get_source_path(need_source=need_source, casename="level0/case12_delay_time001")
    ...
    def test_step(self):
        ...
        while (("1494" in result or "1499" in result) and wait_time <= max_wait_time):
            ...
            time.sleep(1)
            result = driver.System.execute_command("hidumper -ls")
        CheckPoint("1494 and 1499 are unloaded 5 seconds after the screen is turned off")
        assert "1494" and "1499" not in result

    def teardown(self):
        remove_source(source_path=self.source_path, driver=self.driver, sn=self.sn)
        self.log.info("case12_delay_time001 down")
