...
class case17_dysaprofile002(TestCase):
    ...
    def setup(self):
        self.log.info("case17_dysaprofile002 start")
        need_source = {"cfg": True, "listen_test": False, "audio_ability": False, "ondemand": False,
                       "proxy": False, "para": False}
        self.source_path = get_source_path(need_source=need_source, casename="level0/case17_dysaprofile002")
    ...
    def test_step(self):
        ...
        CheckPoint("Query the current loading conditions")
        assert "deviceonline value:off" in result

    def teardown(self):
        remove_source(source_path=self.source_path, driver=self.driver, sn=self.sn)
        self.log.info("case17_dysaprofile002 down")
